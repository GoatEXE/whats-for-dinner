    ...
    >
      <View style={[styles.container, { backgroundColor: c.background }]}>
        <View style={[styles.header, { borderBottomColor: c.surfaceBorder }]}>
          <Text style={[styles.title, { color: c.text }]}>Choose a Meal</Text>
          ...
          renderItem={({ item }) => (
            ...
            >
              <View style={styles.rowContent}>
                <Text style={[styles.mealName, { color: c.text }]} numberOfLines={1}>
                  {item.name}
                </Text>
                {item.tags.length > 0 && (
                  <Text style={[styles.mealMeta, { color: c.textMuted }]} numberOfLines={1}>
                    {item.tags.join(', ')}
                  </Text>
                )}
              </View>
...
const styles = StyleSheet.create({
  ...
  mealName: { fontSize: fontSizes.md, fontWeight: '500' },
  mealMeta: { fontSize: fontSizes.sm, marginTop: 2 },
